...
22. **Overprovisioning_Ratio:** the ratio of reserved storage space with respect to the available flash storage capacity. Range = {all positive double precision values}.
    - **Bad_Block_Retirement_Enabled:** when true, a block that reaches `Block_PE_Cycles_Limit` is permanently retired and is not returned to the free-block pool.
    - **End_of_Life_Threshold:** the remaining OP ratio at or below which the SSD reaches EOL. It must be non-negative and smaller than `Overprovisioning_Ratio`.
...
10. **Relay_Count:** a positive value repeats the trace exactly that many times. A value of zero repeats the full trace until the first SSD reaches EOL; `Percentage_To_Be_Executed` is ignored in this mode. Negative values are invalid.
